	...
	for (i = 0; i < 5; i++) {
		...
		case 0x03:
			...
			if (((SINK_OUI[0] == 0x00) &&
					...
					(SINK_OUI[5] == 0x38) &&
					(SINK_OUI[6] == 0x33))) {
				DEV_NOTICE("DS is 9832 VGA dongle.");
				sp_tx_rx_type = RX_VGA_9832;
			} else {
				sp_tx_rx_type = RX_VGA_GEN;
				DEV_NOTICE("DS is gen DP2VGA converter.");
			}
			ds_port_recoginze = 1;
			break;
		case 0x05:
...
*/
	...
	if (!sp_tx_hw_hdcp_en) {
		...
		if (c & AUTH_EN) {
			for (i = 0; i < 10; i++) {
				sp_read_reg(RX_P1, HDMI_RX_HDCP_STATUS_REG,
						&c);
			if (c & LOAD_KEY_DONE)
				break;
			else
...
static void hdmi_rx_hdcp_done_int(void)
{
	DEV_DBG("*HDMI_RX Interrupt: HDCP is done\n");
	/*if ((!sp_tx_hw_hdcp_en) && (sp_tx_system_state > STATE_HDCP_AUTH))
		sp_tx_set_sys_state(STATE_HDCP_AUTH);*/
}
	...
	if (hdmi_system_state == HDMI_SYNC_DET) {
		...
		if (!(c & TMDS_DE_DET)) {
			if (g_sync_expire_cntr >= SCDT_EXPIRE_TH) {
				...
				/* misc reset */
				sp_read_reg(RX_P0, HDMI_RX_TMDS_CTRL_REG18,
				    &c);
				c |= PLL_RESET;
				sp_write_reg(RX_P0, HDMI_RX_TMDS_CTRL_REG18,
				    c);
				msleep(2);
				sp_read_reg(RX_P0, HDMI_RX_TMDS_CTRL_REG18,
				    &c);
